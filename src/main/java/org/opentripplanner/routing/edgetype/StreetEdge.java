...
public class StreetEdge extends Edge implements Cloneable {
    ...
    public State traverse(State s0) {
        ...
        if (options.kissAndRide || options.rideAndKiss) {
        ...
        else if (options.allowBikeRental) {
            // possible transitions out of renting a bike during "depart at" searches
            if (
                    !options.arriveBy &&
                            s0.isBikeRenting() &&
                            currMode == TraverseMode.BICYCLE
            ) {
                // A StreetEdge has been encountered that does allow bike travel
                //
                ...
                if (!getPermission().allows(TraverseMode.BICYCLE)) {
                    ...
                    if (editorEndedVehicleRental != null) {
                        editorEndedVehicleRental.doneVehicleRenting(); // done with vehicle rental use for now
                        editorEndedVehicleRental.incrementWeight(options.bikeRentalFreeFloatDropoffCost);
                        editorEndedVehicleRental.incrementTimeInSeconds(options.bikeRentalPickupTime);
                ...
                // A StreetEdge has been ecountered where bikes are allowed to traverse
                //
                // In this case, return 2 states:
                // 1. Still renting and riding the vehicle
                // 2. Vehicle rental ended and walking on foot
                else if (getPermission().allows(TraverseMode.BICYCLE)) {
                    StateEditor editorEndedVehicleRental = doTraverse(s0, options, TraverseMode.WALK);
                    if (editorEndedVehicleRental != null) {
                        editorEndedVehicleRental.doneVehicleRenting(); // done with vehicle rental use for now
                        editorEndedVehicleRental.incrementWeight(options.bikeRentalFreeFloatDropoffCost);
            ...
            // possible backward transition of completing a dropping off of a floating bike when in "arrive by" mode
                ...
                StateEditor editorWithVehicleRental = doTraverse(s0, options, TraverseMode.BICYCLE);
    ...
    private StateEditor doTraverse(State s0, RoutingRequest options, TraverseMode traverseMode) {
        ...
        if (backEdge != null && backEdge instanceof StreetEdge) {
            ...
            if (!traverseMode.isDriving()) {
                s1.incrementDistance(realTurnCost / 100, traverseMode);  // just a tie-breaker
            }
        ...
        if (!traverseMode.isDriving()) {
            s1.incrementDistance(getDistance(), traverseMode);
        }
        ...
        if (s1.weHaveCycledTooFar(options)) {
            LOG.debug("Too much cycling. Bailing.");
            return null;
        }
